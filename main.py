...
parser = argparse.ArgumentParser(description='Chebyshev integration of the Lane-Emden equation.')
...
if __name__ == '__main__':
    ...
    ys = [y_0]
    while y_0>0:
        x_0, y_0, z_0 = chebyshev.integrate(x_0, y_0, z_0, n, t_step)
        xs.append(x_0)
        ys.append(y_0)

    # checking that method is valid
    if args.check:
        if n == 0:
            def check(x):
                return 1 - x**2/6
    ...
    print (xs)
    print(ys)
    plt.plot(xs, ys, label = 'Chebyshev')
